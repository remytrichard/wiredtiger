...
static void set_terark_ds(WT_CONNECTION* conn) {
	...
	int ret = conn->add_data_source(conn, "terark:", &trk_dsrc, NULL);
...
start_run(WTPERF *wtperf)
{
	...
	// TBD(kg): terark data-source
	set_terark_ds(wtperf->conn);
	...
	if (wtperf->workers_cnt != 0 &&
		...
		// TBD(kg): after close-reopen, re-add terark data-source
		set_terark_ds(wtperf->conn);
