...
namespace rocksdb {
	...
	bool TerarkBlockIter::BinarySeek(const Slice& target, uint32_t left, uint32_t right,
									 uint32_t* index) {
		assert(left <= right);
	...
	// Return -1 if error.
	int TerarkBlockIter::CompareBlockKey(uint32_t block_index, const Slice& target) {
	...
	// with a key >= target
	bool TerarkBlockIter::BinaryBlockIndexSeek(const Slice& target, uint32_t* block_ids,
	...
	uint32_t TerarkBlock::NumRestarts() const {
		assert(size_ >= 2*sizeof(uint32_t));
		return DecodeFixed32(data_ + size_ - sizeof(uint32_t));
	}

	TerarkBlock::TerarkBlock(TerarkBlockContents&& contents, Statistics* statistics)
	...
	Iterator* TerarkBlock::NewIterator(const Comparator* cmp, TerarkBlockIter* iter,
	...
	size_t TerarkBlock::ApproximateMemoryUsage() const {
		return usable_size();
	}
